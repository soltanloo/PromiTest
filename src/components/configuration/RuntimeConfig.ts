...
export default class RuntimeConfig {
    ...
    constructor(projectPath: string) {
        try {
            logger.debug(
                `Initializing RuntimeConfig with projectPath: ${projectPath}`,
            );
            ...
            logger.info(
                `Successfully loaded project configuration for project: ${projectName}`,
            );
        } catch (e) {
        ...
        logger.info(
            `Project Configuration: ${JSON.stringify(this.config, null, 2)}`,
        );
    }
    ...
    public static getInstance(projectPath?: string): RuntimeConfig {
        if (projectPath) {
            logger.debug(
                `Retrieving instance of RuntimeConfig with projectPath: ${projectPath}`,
            );
            if (RuntimeConfig?.instance?.originalProjectPath === projectPath) {
                logger.debug(
                    `Returning existing instance for projectPath: ${projectPath}`,
                );
                return RuntimeConfig.instance;
            } else {
                logger.debug(
                    `Creating new instance for projectPath: ${projectPath}`,
                );
            ...
            if (!RuntimeConfig.instance) {
                logger.error(
                    'RuntimeConfig is not instantiated and projectPath is missing',
                );
                throw new Error(
                    'RuntimeConfig is not instantiated and projectPath is missing',
                );
    ...
    public setCoverageReportPath(coverageReportPath: string | undefined): void {
