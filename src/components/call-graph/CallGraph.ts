    ...
    private loadCallgraph(callgraphOutput: JSCallgraphOutput) {
        ...
        callgraphOutput.forEach(edge => {
            if (!isNativeFunction(edge.source)) {
                ...
                logger.debug(`Adding node (id: ${node_id}, edge.source: ${JSON.stringify(edge.source)})`);
            }
            ...
            if (!isNativeFunction(edge.target)) {
                ...
                logger.debug(`Adding node (id: ${node_id}, edge.target: ${JSON.stringify(edge.target)})`);
            }
        });
        ...
        callgraphOutput.forEach(edge => {
            if (!isNativeFunction(edge.source) && !isNativeFunction(edge.target)) {
                ...
                logger.debug(`Adding edge (source: ${source_node_id}, target: ${target_node_id})`);
            }
        });
    ...
    private pruneGraph() {
        ...
        this.nodes.forEach((_, nodeId) => {
            if (!visited.has(nodeId)) {
                logger.debug(`Removing node (id: ${nodeId}) as it is not reachable from tests.`);
        ...
        this.adjacencyList.forEach((neighbors, nodeId) => {
            const filteredNeighbors = neighbors.filter(neighbor => visited.has(neighbor.id));
            if (filteredNeighbors.length < neighbors.length) {
                logger.debug(`Updating neighbors for node (id: ${nodeId}). Before: ${neighbors.length}, After: ${filteredNeighbors.length}`);
            }
