...
export default class TestValidator {
    ...
    static async validateRuntime(promiseId: NodeId, flag: string): Promise<boolean> {
        ...
        logger.info(`Validating runtime for promise ${promiseId} with flag ${flag}`);
        mocha.addFile(path.resolve(TestGenerator.getTestFilePathForPromise(promiseId, flag)));
        logger.debug(`Added test file to mocha: ${TestGenerator.getTestFilePathForPromise(promiseId, flag)}`);
        try {
            ...
            return new Promise((resolve, reject) => {
                mocha.run((failures) => {
                    if (failures) {
                        logger.error(`Test failed for promise ${promiseId} with flag ${flag}`);
                        resolve(false);
                    } else {
                        logger.info(`Test passed for promise ${promiseId} with flag ${flag}`);
                        resolve(true);
                    }
                });
            });
        } catch (error) {
            logger.error(`Error running test for promise ${promiseId} with flag ${flag}`);
            throw error;
        }
